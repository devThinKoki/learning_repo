...
try:
    ...
    print('ok! else!') # try문이 정상 실행된 경우에는 실행되는 구문
...
# 이렇게 구문 처리를 할 경우, 위의 코드가 실행된 후 반드시 아래코드가 실행되도록 해줌.
try:
    print('try')
finally:
    print('finally')
...
except Exception: # Exception은 모든 종류의 예외를 잡아내기 때문에 가장 마지막에 작성해주는 것이 좋다.
...
try:
    a = 'kim'
    if a == 'kim':
        print("ok! You're kim! Pass!")
    else:
        raise ValueError # a가 'kim'이 아닌 경우 자체적으로 에러를 발생시키기
except ValueError:
